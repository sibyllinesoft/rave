...
Start by editing the `global` layer (it already contains a sample Traefik snippet and
...
`rave overrides create-layer <name> --priority 50 --preset traefik` rather than building the
tree by hand—each layer gets its own `layer.json`, `metadata.json`, and mirror directories.
Presets (`traefik`, `gitlab`, `mattermost`, `pomerium`, `authentik`) append common restart/reload patterns so
...
- `global` – default shadow filesystem for all hosts (sample Traefik + systemd snippets).
