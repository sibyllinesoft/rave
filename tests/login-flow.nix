...
testLib.runTest {
  ...
  testScript = ''
    ...
    machine.wait_for_unit("traefik.service", timeout=600)

    machine.wait_until_succeeds("systemctl is-active gitlab.service", timeout=600)
    machine.wait_until_succeeds("systemctl is-active traefik.service", timeout=600)

    machine.wait_until_succeeds(
      "curl -k -sSf --max-time 45 https://localhost/gitlab/users/sign_in",
      timeout=600
    )
    machine.wait_until_succeeds(
      "curl -k -sSf https://localhost/gitlab/users/sign_in | grep -q 'google_oauth2'",
      timeout=600
    )
    machine.wait_until_succeeds(
