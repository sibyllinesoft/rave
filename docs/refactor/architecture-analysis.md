...
- There is no single `src/` or `packages/` boundary; instead, functionality is split across `apps/cli/` (Python), `services/chat-control` (Python), `services/mattermost-bridge` (Python), `infra/nixos/` (NixOS modules), `scripts/`, `build-scripts/`, `demo-scripts/`, and the historical `legacy/configs/` tree.
...
- Manual shell helpers in `build-scripts/` duplicate some flake responsibilities (`build-vm.sh`, `build-vms.sh`, etc.), which can drift from the CLI’s automation path (`apps/cli/vm_manager.py`) and add to cognitive load.
...
- **Inconsistent Tooling Paths**: Multiple orchestration interfaces (`apps/cli/rave`, `build-scripts/*.sh`, `legacy/configs/run*.sh`, documentation for Docker Compose) coexist, with no authoritative matrix of which commands remain supported.
