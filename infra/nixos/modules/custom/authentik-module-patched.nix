...
let
  ...
  # nixpkgs 24.05 compatibility: `types.pathWith` is unavailable, so use plain path handling for secrets.
  pathToSecret = types.path;
in
{
  options.services = {
    # authentik server
    authentik = {
      ...
      authentikComponents = mkOption {
        default = pkgs.authentikComponents;
