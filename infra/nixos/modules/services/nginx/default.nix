...
'';
  ...
  primaryListen =
    if behindPomerium then
      [
        { addr = "127.0.0.1"; port = backendPort; ssl = false; }
      ]
    else
      [
  ...
  primaryVirtualHost = {
    forceSSL = !behindPomerium;
    enableACME = false;
    listen = primaryListen;
    ...
    locations = mkMerge (
      ...
      ++ optional (pomeriumEnabled && !behindPomerium) (
        let
...
let
  commonOptions = {
    ...
    behindPomerium = mkOption {
      type = types.bool;
      default = false;
      description = "When true, expose nginx only on loopback and expect Pomerium to terminate TLS.";
    };

    backendPort = mkOption {
      type = types.int;
      default = 9443;
      description = "Loopback port used when nginx runs behind Pomerium.";
    };
...
in
{
  ...
  options.services.rave.traefik = commonOptions;
    ...
    (mkIf (mattermostEnabled && !behindPomerium) {
    ...
    (mkIf (mattermostEnabled && chatDomain != null && !behindPomerium) (
      let
        chat = chatDomain;
      in {
